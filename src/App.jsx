...
export default function SeventyFiveHardGuide() {
  ...
  const downloadCertificate = () => {
    ...
    doc.setFontSize(10);
    doc.text("75 HARD TRANSFORMATION JOURNAL", centerX, 78, { align: "center" });

    doc.setTextColor("#B76E79");
    doc.setFont("times", "bold");
    doc.setFontSize(34);
    doc.text("Certificate of Completion", centerX, 122, { align: "center" });

    doc.setTextColor("#6B5F5F");
    doc.setFont("helvetica", "normal");
    doc.setFontSize(14);
    doc.text("This certifies that", centerX, 162, { align: "center" });

    doc.setTextColor("#4A4A4A");
    doc.setFont("times", "normal");
    doc.setFontSize(30);
    doc.text(trackerName || "Participant Name", centerX, 205, { align: "center" });

    doc.setDrawColor("#D4AF37");
    doc.line(centerX - 180, 216, centerX + 180, 216);

    doc.setTextColor("#6B5F5F");
    doc.setFont("helvetica", "normal");
    doc.setFontSize(14);
    doc.text("successfully completed the", centerX, 250, { align: "center" });

    doc.setTextColor("#B76E79");
    doc.setFont("times", "bold");
    doc.setFontSize(26);
    doc.text("75 Hard Challenge", centerX, 285, { align: "center" });

    doc.setTextColor("#6B5F5F");
    doc.setFont("helvetica", "normal");
    doc.setFontSize(13);
    doc.text("through 75 consecutive days of discipline, consistency, resilience, faith,", centerX, 320, { align: "center" });
    doc.text("personal growth, and promises kept.", centerX, 340, { align: "center" });

    const stats = [["75", "DAYS"], ["150", "WORKOUTS"], ["1,500", "PAGES"], ["75", "GALLONS"]];
    const startX = centerX - 250;
    stats.forEach(([number, label], index) => {
      const x = startX + index * 165;
      doc.setDrawColor("#D4AF37");
      doc.setFillColor("#FFFFFF");
      doc.roundedRect(x, 370, 120, 70, 14, 14, "FD");
      doc.setTextColor("#D4AF37");
      doc.setFont("times", "bold");
      doc.setFontSize(24);
      doc.text(number, x + 60, 400, { align: "center" });
      doc.setTextColor("#9F5F6B");
      doc.setFont("helvetica", "bold");
      doc.setFontSize(9);
      doc.text(label, x + 60, 420, { align: "center" });
    });

    doc.setFillColor("#D4AF37");
    doc.circle(centerX, 455, 36, "F");
    doc.setTextColor("#FFFFFF");
    doc.setFont("helvetica", "bold");
    doc.setFontSize(10);
    doc.text("PROMISE", centerX, 452, { align: "center" });
    doc.text("KEPT", centerX, 468, { align: "center" });

    doc.setTextColor("#4A4A4A");
    doc.setFont("times", "italic");
    doc.setFontSize(13);
    doc.text("“I have fought the good fight, I have finished the race, I have kept the faith.”", centerX, 520, { align: "center" });
    doc.setTextColor("#B76E79");
    doc.setFont("helvetica", "bold");
    doc.setFontSize(10);
    doc.text("2 Timothy 4:7", centerX, 538, { align: "center" });

    doc.setTextColor("#6B5F5F");
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    doc.line(150, 565, 330, 565);
    doc.text("Completion Date", 240, 580, { align: "center" });
    doc.line(pageWidth - 330, 565, pageWidth - 150, 565);
    doc.text("Signature", pageWidth - 240, 580, { align: "center" });

    doc.setTextColor("#9F5F6B");
    doc.setFont("helvetica", "bold");
    doc.setFontSize(8);
    doc.text(`Certificate No. ${certificateNumber}`, centerX, 595, { align: "center" });

    doc.setTextColor("#D4AF37");
    doc.setFont("times", "italic");
    doc.setFontSize(10);
    doc.text("A promise kept becomes confidence earned.", centerX, 607, { align: "center" });
